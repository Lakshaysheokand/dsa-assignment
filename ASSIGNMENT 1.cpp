...
// SOLUTON -
// We can maintain a check that the varaible pointer , whenver it points to the adress same as that of the head node it must stop.
...
4. They are also used for streaming video. The server sends us chunks of video and sound at kinda unpredictable rates - maybe every few seconds, another chunk of video appears that’s a second or so long, so we stuff it into a queue.
5. If your network connection is a bit slow - then it’s possible that the part that’s taking data out of the queue manages to empty it completely - and you have to display a “BUFFERING…” message.
*/